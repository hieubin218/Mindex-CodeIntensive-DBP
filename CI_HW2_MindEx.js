...
console.log(reverseNumber()); // null --> 0
...
// Convert a time in 12-hour AM/PM format to Military (24-hour) time.
    // Note: 12:00:00PM = 00:00:00 (24-hour clock)

const timeConversion = function (Timestring) {
    const TimeType = Timestring.slice(-2);
    const TimeHour = +(Timestring.slice(0, 2));
    let time = Timestring.slice(0, -2);

    if (TimeType === "AM") {
        if (TimeHour === 12) {
            return time.replace(TimeHour, "00");
        };
        return time;
    } else if (TimeType === "PM") {
        if (TimeHour !== 12) {
            return time.replace(Timestring.slice(0, 2), String(TimeHour + 12));
        }
        return time;
    }
}
...
// timeConversion("08:21:01AM");

console.log(timeConversion("08:21:01AM"));
console.log(timeConversion("07:31:24PM"));
console.log(timeConversion("09:23:21PM"));